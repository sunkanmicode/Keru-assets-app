...
import BuyAirtimeBottomSheet from "../BuyAirtimeBottomSheet";

const BuyAirtimeReviewComp = () => {
  const navigation = useNavigation();
  const BuyAirtimeSheetRef = React.useRef(null);

  const height = 343;

  const buyAirtimeCloseSheet = () => {
    if (BuyAirtimeSheetRef.current) {
      BuyAirtimeSheetRef.current.close();
    }
  };
  const buyAirtimeOpenSheet = () => {
    if (BuyAirtimeSheetRef.current) {
      BuyAirtimeSheetRef.current.open();
    }
  };
  return (
    <>
      ...
      <CustomBottomSheet
        height={height}
        ref={BuyAirtimeSheetRef}
        message={<BuyAirtimeBottomSheet />}
      />
    </>
  );
};

export default BuyAirtimeReviewComp;
